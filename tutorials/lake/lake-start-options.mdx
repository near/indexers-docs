...
- `Opts` and `ChainId` are boilerplate structs we will cover in [Designing the `StartsOpts`](#designing-the-startoptions) section
...
So we want to be able to pass a command that defines the way our indexer should start.

We already have the structure `Opts` that receives `ChainId` enum variant. This allows us to use command:
...
Now we want to create `StartOptions` structure that will allow us to tell our indexer where to start indexing from. The command should look like:
