---
sidebar_position: 4
sidebar_label: Lake indexer Start options
---
...
Also, during [the April Data Platform Community Meeting](/blog/2022-06-11-community-meeting-record) we had a question whether we plan to add this feature to the library. We've promised to create a tutorial showing how to do it by your own. So here it is.
...
- `LakeConfigBuilder` has been imported to craft `LakeConfig` for indexer
- `Opts` and `ChainId` are boilerplate structs to allow us to run a command for our indexer like the following:
  ```bash
...
So we want to be able to pass a command that defines the way our indexer should start. Our variants are:
...
We are marking `lake_config_builder` as mutable for a reason.
...
It's not the behaviour we expect. That's why we assume you want it to start from interruption (if possible) or from the latest.
...
You've seen the way how you can empower your indexer with the starting options.
