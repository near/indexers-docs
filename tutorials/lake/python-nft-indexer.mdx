...
- `receipt_id` of the [Receipt](/docs/data-flow-and-structures/structures/receipt) where the mint has happened
- Marketplace
...
Just a few words on the config, function `mainnet()` has set `s3_bucket_name`, `s3_region_name`for mainnet and recent block height for `start_block_height`. You can go to [NEAR Explorer](https://explorer.near.org) and get **the most recent** block height for your setup. Though you can use the same as we do.
...
We have iterated through the logs of all ExecutionOutcomes of [Shards](/docs/data-flow-and-structures/structures/shard) (in our case we don't care on which Shard did the mint happen)
