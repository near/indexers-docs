...
This article relies on the assumption you are familiar with the [NEAR Data Flow](./near-data-flow.mdx) article. We build upon its concepts for further explanation.
...
If you don't know what **Gas** is, please [read first the article about Gas](https://docs.near.org/docs/concepts/gas) from our docs.
...
Assume we have two accounts **alice.near** and **bob.near**. They belong to different [Shards](../structures/shard.mdx). **alice.near** sends a few tokens to **bob.near**.

A [Transaction](../structures/transaction.mdx) signed by **alice.near** is sent to the network. It is immediately executed, [ExecutionOutcome](../structures/execution_outcome.mdx) is the output or result from converting the transaction into a [Receipt](../structures/receipt.mdx).

![Transaction execution](/docs/flow/03-tx-outcome-receipt.png)

During the above process **alice.near**, the sender, was charged a fee (gas). The [Receipt](../structures/receipt.mdx) created as result of the [Transaction](../structures/transaction.mdx) follows these rules:
...
Almost done. Remember the refund? So the [ExecutionOutcome](../structures/execution_outcome.mdx) for the Receipt will be another Receipt that is refunding the Gas to the sender. **bob.near** has received tokens from **alice.near**. Now, **alice.near** becomes the receiver for a new (and last) Receipt (keep in mind the sender in this Receipt is always **system**).
...
Let's have a look the example where both accounts are on the same [Shard](../structures/shard.mdx). The process is the same as in the previous example, except there are no Receipts moving from one Shard to another.

A [Transaction](../structures/transaction.mdx) signed by **alice.near** is sent to the network. It is immediately executed, [ExecutionOutcome](../structures/execution_outcome.mdx) is the result of converting the transaction into a [Receipt](../structures/receipt.mdx).

![Transaction execution](/docs/flow/03-tx-outcome-receipt.png)

The Receipt is already on the receiver's Shard, so it is put in the execution queue of the next [Block](../structures/block.mdx). It is executed in the next Block, and the [ExecutionOutcome](../structures/execution_outcome.mdx) result is a new Receipt with the refund to the initial sender, **alice.near**. 
