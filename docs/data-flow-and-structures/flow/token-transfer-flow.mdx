...
Basically it is an extention of the example from the [NEAR Data Flow](./near-data-flow.mdx) article.
...
This is it. Tokens has been transfered from the account from one Shard to the account from the different Shard and the initial sender received the refund of Gas.


## Token transfer between the accounts from the same shard

Let's have a look the example where both accounts belong to the same [Shard](../structures/shard.mdx). The proces is completely the same as in the previous example, except the Receipts moving from one Shard to another.
